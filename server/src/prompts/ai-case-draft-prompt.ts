...
export function buildAiCaseDraftSystemPrompt(uiLibrary: UiLibrary = 'auto') {
  return [
    ...
    '- 优先使用 pageContext.fields 里唯一且可执行的 locator，字段名只作为匹配语义，不默认等于可访问名。',
    '- fields 有 locators 时优先原样使用最高置信且唯一的候选 selector，且应优先选择短、可执行、可直接验证的定位器。',
    '- 不要把 pageContext.fields 中的长 XPath 作为最终候选输出；若存在更短的 input#id、input[name]、locator(\'...\') 等可执行定位器，应优先选它们。',
    '- 页面上下文没有可匹配 locator 时，可以基于自然语言尝试推理 Playwright selector。',
    '- 推理 selector 时不要默认把 targetType=input/select 直接映射成 getByRole("textbox"/"combobox", { name })；只有在 pageContext 里没有更短且唯一可执行的 locator，且 DOM 证据支持该可访问名时才这样做。',
