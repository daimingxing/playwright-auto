...
- 状态：已解决
- 问题：Kendo 下拉控件本体经常只有当前显示值，例如 `---请选择---`，字段名在同一 `.xr-fc`、`.i-select` 等字段容器内的 `label` 中。只按控件可访问名称或当前文本采集时，会把当前值误当作字段名，AI 也容易生成不可靠的 `getByLabel('---请选择---')`。
- 处理：页面上下文增加 `PageContext.fields` 字段语义层，Kendo 采集从控件反向查找最近字段容器并读取容器内 label，当前显示值只进入 `value`；AI 摘要和 selector 补全优先消费 `fields`；页面地图详情接口从 snapshot 展开 `fields` 到状态响应，抽屉展示字段名、类型、UI、当前值、首选 selector、唯一性和来源置信度，方便排查采集质量。
- 经验：Kendo 字段归属应以“字段容器 + label + 控件”作为整体判断，不应把控件当前值作为字段名或首选定位依据。Kendo 下拉 options 展开采集本次暂未实现，后续需要通过 `aria-controls` 等关系只读展开选项并继续保持安全探索边界。
...
## 2026-05-30 手动登录页加载超时后无法保存登录态

- 状态：已解决
- 问题：运行中心点击“打开浏览器登录”后，后端已经创建浏览器会话，但目标系统页面加载超过 Playwright 默认 30 秒导航超时，`/auth/start` 返回 500。用户之后即使在已打开浏览器中进入目标系统，前端也没有拿到 `sessionId`，导致“我已完成登录，保存登录态”按钮一直禁用。
- 处理：`server/src/services/auth-session.ts` 将登录页自动打开失败降级为 warning，保留已创建的会话并返回 `sessionId`；前端收到 warning 时提示用户手动刷新或输入地址，同时保留保存按钮可用。保存成功后避免重复关闭浏览器。
- 经验：手动登录流程的关键资产是浏览器会话，不是初始 URL 自动打开成功。慢内网、网关跳转或目标系统偶发超时不能让用户丢失可保存的登录会话。

---

## 2026-05-30 全量测试受本地 Excel 模板改动影响

- 状态：待解决
- 问题：抽取用例步骤渲染共享工具后执行 `npm run test`，只有 `tests/web/ai-import.test.ts` 中“真实 Excel 模板可以被解析为两表导入数据”和“真实 Excel 模板保留两张业务表和中文英文动作值”失败。失败信息显示真实模板只解析到 1 条用例，步骤表第 3 列也缺少期望的 `检查可见(assertVisible)`。开工前 `docs/ai-case-import/AI自然语言用例导入模板.xlsx` 已处于已修改状态，本次改动未接触 AI 导入解析链路。
- 处理：保留模板和测试现状，不在本次重复实现修复中混入 Excel 模板修复；用受影响范围内的服务端测试和类型检查验证本次改动。
- 经验：全量测试依赖真实二进制模板文件时，若模板已有未提交改动，需要先区分当前任务改动和本地样例数据改动，避免把无关 fixture 漂移混入代码重构。

---

## 2026-05-30 页面地图打开超时和失败重试复用旧状态

- 状态：已解决
- 问题：页面地图和手动登录分别维护打开业务 URL 的等待语义，页面地图还把该等待放在 AI 页面地图配置下，容易和 `ai.timeoutMs`、`steps.timeouts` 混淆。页面地图初始 `domcontentloaded` 超时会直接失败，即使页面已有可读内容也不会继续采集；页面地图生成失败后导入项会清空 `pageMapId`，后续重试可能绕过刷新成功的新页面地图。
- 处理：新增 `browser.openTimeoutMs` 统一控制平台打开业务 URL 的等待上限，手动登录初始打开和页面地图初始采集共用；移除页面地图配置中的打开等待字段。页面地图初始导航遇到 `domcontentloaded` 超时会记录 warning，并继续做有限 ready 等待和快照读取，HTTP 400 及以上仍按不可访问失败。单项重试失败导入项时，会按项目、环境、目标 URL、登录态和控件库重新解析页面地图；命中旧 failed 地图时先刷新，刷新成功后绑定并复用新快照。
- 经验：平台自身打开业务 URL、模型请求、测试步骤等待是三类不同超时，配置命名必须直接体现职责。失败缓存不能长期驱动重试路径，否则用户刷新成功后仍会被旧失败状态困住。

---

## 2026-05-30 页面地图前置表单入口动作被过滤

- 状态：已解决
- 问题：cctq 导入步骤已经包含“令牌管理 -> 添加令牌 -> 名称 -> 令牌分组”，但页面地图只生成“初始页面”和“令牌分组探索失败”。根因不是“添加令牌”按钮 selector 找不到，而是安全动作识别把表单字段前的 `table` 入口和普通“添加”按钮都过滤掉，导致浏览器停留在初始页后直接尝试找下拉字段。
- 处理：表单字段出现前允许非危险入口点击进入页面地图路径，例如“管理/列表/详情”类 table/link/text/region，以及“添加/新增/创建/编辑/设置”等 button；危险词仍优先拦截。页面地图缓存键增加安全探索动作摘要，避免同 URL 的旧初始页缓存复用到需要打开表单的导入项；页面地图刷新接口会从关联导入项找回原始步骤再刷新。
- 经验：字段语义为空不等于页面地图详情为空，可能是状态快照停在错误页面或未进入表单态。排查页面地图时要同时看 `states.sourceAction`、warnings 和 snapshot 的 `fields`，不能只按某个 selector 是否存在判断根因。
